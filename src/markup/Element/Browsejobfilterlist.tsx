...
function Browsejobfilterlist() {
    ...
    const [getFilterJob, { isLoading: isFilterLoading, data: jobsData }] = useGetFilterJobMutation();

    console.log('jobsData', jobsData)
    useEffect(() => {
        if (jobTitleQuery || cityQuery || sectorQuery) {
            getFilterJob({ job_title: jobTitleQuery, city: cityQuery, sector: sectorQuery });
        }
                        ...
                        <div className="row">
                            ...
                            <div className="col-xl-9 col-lg-8 col-md-7">
                                <div className="job-bx-title clearfix">
                                    <h5 className="font-weight-700 pull-left text-uppercase">{`${jobsData?.data?.length || 0} Jobs Found`}</h5>
                                            ...
                                            <div className="post-bx">
                                                <div className="d-flex m-b30">
                                                    <div className="job-post-info">
                                                        <h4><Link href={"/job-detail"}> {item?.job_title}</Link></h4>
                                                        <ul>
                                                            <li><i className="fa fa-map-marker"></i> {item?.address}</li>
                                                            <li><i className="fa fa-bookmark-o"></i>{item?.location?.title}</li>
                                                            <li><i className="fa fa-clock-o"></i>  Published{" "}
                                                            {formaterDate(item?.created_at)}</li>
                                                        </ul>
                                                    </div>
                                                </div>
                                                <div className="d-flex">
                                                    <div className="job-time mr-auto">
                                                        <Link href={"#"}><span>{item?.location?.title}</span></Link>
                                                    </div>
