  ...
  useEffect(() => {
    if (resumeData && resumeData.data.length > 0) {
      ...
      if (existingSkills.length > 0) {
        setSkills(existingSkills.map((skill: any) => skill.title).join(", "));
  ...
  const handleSave = async () => {
    ...
    try {
      if (editMode && keySkillId !== null) {
        ...
        const response = await updateKeySkill({
          data: skillData,
        ...
        const response = await createKeySkill(skillData);
        toast.success(response?.data?.message, { theme: "colored" });
      }
      onHide();
    } catch (error: any) {
    ...
    <div id="key_skills_bx" className="job-bx bg-white m-b30">
      ...
      <div className="job-time mr-auto">
        {skills.split(",").map((skill, index) => (
          <Link href="" className="mr-1" key={index}>
