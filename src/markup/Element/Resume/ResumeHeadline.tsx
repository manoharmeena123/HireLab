...
import { useGetResumeDataQuery, useCreateResumeHeadlineMutation, useUpdateResumeHeadlineMutation } from "@/app/my-resume/store/resume.query";
...
const ResumeHeadline: React.FC<ResumeHeadlineProps> = ({ show, onShow, onHide }) => {
    ...
    const [headline, setHeadline] = useState<WritableHeadlineData>({ description: '' });
    ...
    const handleSave = async () => {
        if (editMode && headlineId !== null) {
            ...
            await updateHeadline({ data: headline, headline_id: headlineId });
        } else {
            // Create new headline
            await createHeadline(headline);
        }
        onHide();
    };
    ...
    return (
        <div id="resume_headline_bx" className="job-bx bg-white m-b30">
            <div className="d-flex">
                ...
                <Link href='' className="site-button add-btn button-sm" onClick={onShow}>
                    <i className="fa fa-pencil m-r5"></i> Edit
                </Link>
            </div>
            <p className="m-b0">{headline.description || 'Job board currently living in USA'}</p>
                                    ...
                                    <div className="col-lg-12 col-md-12">
                                        <div className="form-group">
                                            <textarea
                                                ...
                                                onChange={(e) => setHeadline({ ...headline, description: e.target.value })}
                                            ></textarea>
                                        </div>
                                    </div>
                                </div>
